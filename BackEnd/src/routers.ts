...
router.post('/change-password', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const userQuery = 'SELECT user_id, user_password FROM users WHERE email = $1'; // Updated to use user_password
    ...
    const isMatch = await bcrypt.compare(oldPassword, user.user_password); // Updated to use user_password
    if (!isMatch) {
    ...
    const updateQuery = 'UPDATE users SET user_password = $1 WHERE user_id = $2'; // Updated to use user_password
    await pool.query(updateQuery, [hashedPassword, user.user_id]); // Ensure we're using user_id
